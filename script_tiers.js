...
tiers.commands = {
	tierscommands: function(src, channel, command){
		var osymbol = global.auth === undefined ? "" : auth.options["owner"].image, usymbol = global.auth === undefined ? "" : auth.options["user"].image;
	...
	importtiers: function(src, channel, command){
		...
		var filename =  command[1] === "undefined" ? tiers.options.name + ".xml" : command[1] + ".xml", next_tiers = sys.getFileContent(filename), current_tiers = sys.getFileContent("tiers.xml");
